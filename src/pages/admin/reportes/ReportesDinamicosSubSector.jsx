...
import itemsForReportsConfig from './config/itemsForReportsSector';
...
const ReportesDinamicosEventosActividad = () => {
  ...
  const handleExportExcel = async () => {
    ...
    await exportEmpresasToExcel(reporteData, null, null, columnasSeleccionadas, 'LISTADO DE EMPRESAS POR SUB - SECTOR PRODUCTIVO');
  };
  ...
  const handleExportPDF = () => {
    // Filtrar solo los campos seleccionados (checked)
    const columnasSeleccionadas = itemsForReports.filter(item => item.checked);
    exportEmpresasToPDF(reporteData, null, null, columnasSeleccionadas, 'LISTADO DE EMPRESAS POR SUB - SECTOR PRODUCTIVO');
  };
  ...
  return (
    <div>
      <div className='bg-white p-4 rounded-2xl mt-4'>
        <AtomsTitle
          title={'LISTADO DE EMPRESAS POR SUB - SECTOR PRODUCTIVO'}
          subtitle={'Reporte de empresas por subsector productivo registrado'}
          className="mb-4"
        />
        <Form {...form} >
