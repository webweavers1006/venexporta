    ...
    <Suspense fallback={<Loading />}>
      ...
      <div className="flex flex-1 flex-col gap-4 mt-4">
        ...
        <div className="grid auto-rows-min gap-4 md:grid-cols-3">
          ...
          <div className="col-span-1">
            {cantSubSector && Array.isArray(cantSubSector.subSectoresEconomicos) && (
              <MoleculesChartPie
                data={cantSubSector.subSectoresEconomicos.filter(item => item.sub_sector_productivo).map(item => ({
                  label: item.sub_sector_productivo,
                  value: item.total,
                  fill: item.color || undefined
                }))}
                config={cantSubSector.subSectoresEconomicos.filter(item => item.sub_sector_productivo).reduce((acc, item) => {
